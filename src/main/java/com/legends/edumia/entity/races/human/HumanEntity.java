...
public class HumanEntity extends NpcEntity {
    ...
    protected ResourceLocation getFactionId() {
        return EdumiaFactions.HUMAN.getId();
    }

    @Override
    protected ResourceLocation getRaceId() {
        return EdumiaRaces.HUMAN.getId();
    }
    ...
    protected void actuallyHurt(DamageSource damageSource, float damageAmount) {
        if (damageSource.getEntity() instanceof HumanEntity){
            return;
        }
        super.actuallyHurt(damageSource, damageAmount);
    }

    public HumanVariant getVariant(){
        return HumanVariant.byId(this.getId());
    }

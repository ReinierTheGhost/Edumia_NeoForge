...
public class OgreEntity extends NpcEntity {
    ...
    protected ResourceLocation getFactionId() {
        return EdumiaFactions.OGRES.getId();
    }

    @Override
    protected ResourceLocation getRaceId() {
        return EdumiaRaces.OGRE.getId();
    }
    ...
    protected void actuallyHurt(DamageSource damageSource, float damageAmount) {
        if (damageSource.getEntity() instanceof OgreEntity){
            return;
        }
        super.actuallyHurt(damageSource, damageAmount);
    }

    public OgreVariant getVariant(){
        return OgreVariant.byId(this.getId());
    }

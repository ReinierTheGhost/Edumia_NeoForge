...
public class DemonEntity extends NpcEntity {
    ...
    protected ResourceLocation getFactionId() {
        return EdumiaFactions.DEMONS.getId();
    }

    @Override
    protected ResourceLocation getRaceId() {
        return EdumiaRaces.DEMON.getId();
    }
    ...
    protected void actuallyHurt(DamageSource damageSource, float damageAmount) {
        if (damageSource.getEntity() instanceof DemonEntity){
            return;
        }
        super.actuallyHurt(damageSource, damageAmount);
    }

    public DemonVariant getVariant(){
        return DemonVariant.byId(this.getId());
    }
